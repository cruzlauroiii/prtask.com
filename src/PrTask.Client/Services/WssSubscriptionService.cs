...
public class WssSubscriptionService(ILocalStoreService Store) : ISubscriptionService
{
    ...
    public async Task<IReadOnlyList<SubscriptionEntity>> ListSubscriptionsAsync(string CustomerId, string CardTokenId) =>
        (await Store.GetRecordsAsync<SubscriptionEntity>(OpfsConstants.CollectionSubscriptions))
            .Where(S => S.CustomerId == CustomerId).ToArray();
