...
internal sealed class AnthropicApiClient : IDisposable
{
    ...
    internal AnthropicApiClient(string Token, bool IsApiKey = false, string? ProxyBaseUrl = null)
    {
        Client = new HttpClient();
        if (ProxyBaseUrl is not null)
        {
            MessagesEndpoint = string.Concat(ProxyBaseUrl, "/v1/messages");
        }
        else
        {
        ...
        CancellationToken Ct = default)
    {
        ...
        using var Response = await Client.PostAsync(MessagesEndpoint, Content, Ct).ConfigureAwait(false);
