...
public static class GitPack
{
    ...
    public static Dictionary<string, PackIndexEntry> ReadIndex(string IdxPath)
    {
        ...
        for (uint I = 0; I < TotalObjects; I++)
        {
            ...
            {
                // 64-bit offset table follows the 32-bit offset table
                ...
                if (Offset64TableOffset + (Idx64 * 8) + 8 <= Data.Length)
                {
                    ...
                    PackOffset = Low; // Truncate to 32-bit for now
                }
            }
    ...
    public static byte[] CreatePack(GitRepository Repo, string TipHash, string? BaseHash)
    {
        ...
        // If we have a base, collect its objects so we skip them
        if (BaseHash is not null)
        {
            CollectReachableObjects(Repo, BaseHash, BaseObjects);
        }

        // Collect all objects reachable from tip that aren't in base
    ...
    private static GitRawObject? ReadPackObject(byte[] Data, ref int Offset)
    {
        ...
        var ObjType = TypeBits switch
        {
            ...
            4 => GitObjectType.Tag,
            6 => GitObjectType.Blob, // OFS_DELTA - simplified
            7 => GitObjectType.Blob, // REF_DELTA - simplified
        ...
        if (TypeBits == 6)
        {
            ...
            // Simplified: store raw delta data as blob
        ...
        if (TypeBits == 7)
        {
            // REF_DELTA: read base object hash (20 bytes)
        ...
        // Regular object: decompress zlib data
    ...
    private static byte[]? DecompressData(byte[] Data, ref int Offset, int ExpectedSize)
    {
        try
        {
            ...
            // ZLibStream doesn't tell us how many bytes were consumed,
            // so estimate based on the input stream position
        ...
        {
            // Skip past estimated compressed size
