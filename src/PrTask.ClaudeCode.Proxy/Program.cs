...
App.MapPost("/api/tool/bash", async (HttpContext Ctx) =>
{
    ...
    catch (OperationCanceledException) { Proc.Kill(true); }
    var StdOut = await Proc.StandardOutput.ReadToEndAsync();
    var StdErr = await Proc.StandardError.ReadToEndAsync();
    var Output = StdOut.Length > 0 && StdErr.Length > 0 ? string.Concat(StdOut, "\nSTDERR:\n", StdErr) : StdOut.Length > 0 ? StdOut : StdErr;
...
App.MapPost("/api/tool/read", async (HttpContext Ctx) =>
{
    ...
    var Offset = 0;
    var Limit = 2000;
    if (Doc.RootElement.TryGetProperty("offset", out var OffProp)) Offset = OffProp.GetInt32();
    if (Doc.RootElement.TryGetProperty("limit", out var LimProp)) Limit = LimProp.GetInt32();
...
App.MapPost("/api/tool/edit", async (HttpContext Ctx) =>
{
    ...
    if (!File.Exists(FilePath)) { Ctx.Response.StatusCode = 404; await Ctx.Response.WriteAsync(JsonSerializer.Serialize(new { error = "File not found" })); return; }
    var FileContent = await File.ReadAllTextAsync(FilePath);
    if (!FileContent.Contains(OldString)) { Ctx.Response.StatusCode = 400; await Ctx.Response.WriteAsync(JsonSerializer.Serialize(new { error = "old_string not found" })); return; }
    await File.WriteAllTextAsync(FilePath, FileContent.Replace(OldString, NewString));
...
App.MapPost("/api/tool/glob", async (HttpContext Ctx) =>
{
    ...
    var SearchPath = Cwd;
    if (Doc.RootElement.TryGetProperty("path", out var PathProp)) SearchPath = PathProp.GetString() ?? Cwd;

    var Psi = new ProcessStartInfo("cmd.exe", string.Concat("/c dir /s /b \"", Path.Combine(SearchPath, Pattern), "\" 2>NUL"))
    { WorkingDirectory = SearchPath, RedirectStandardOutput = true, UseShellExecute = false, CreateNoWindow = true };
    using var Proc = Process.Start(Psi)!;
