...
class p9785642f$11 : androidx.room.EntityDeletionOrUpdateAdapter<p8325324b.p07214c67.pd77d5e50.p32a285cf> {
    readonly p8325324b.pf0719ea8.p9785642f this$0;

    p9785642f$11(p8325324b.pf0719ea8.p9785642f p9785642fVar, androidx.room.RoomDatabase roomDatabase) {
        super(roomDatabase);
    ...
    protected void Bind2(androidx.sqlite.db.SupportSQLiteStatement supportSQLiteStatement, p8325324b.p07214c67.pd77d5e50.p32a285cf p32a285cfVar) {
        ...
        supportSQLiteStatement.bindstring(13, p32a285cfVar.getUuid());
        supportSQLiteStatement.bindstring(14, p32a285cfVar.getReceiptUuid());
    }

    protected override java.lang.string CreateQuery() {
        return "UPDATE OR ABORT `current_sell_payment` SET `uuid` = ?,`receiptUuid` = ?,`paymentType` = ?,`amount` = ?,`tapOnPhoneRefId` = ?,`transaction_rrn` = ?,`transaction_stan` = ?,`transaction_authCode` = ?,`transaction_responseCode` = ?,`transaction_topRefUuid` = ?,`transaction_terminalId` = ?,`transaction_merchantId` = ? WHERE `uuid` = ? AND `receiptUuid` = ?";
    }
}
