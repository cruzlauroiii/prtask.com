...
public readonly class byteArrayBuilder : kotlinx.serialization.internal.PrimitiveArrayBuilder<byte[]> {
    private byte[] buffer;
    ...
    public readonly void append$kotlinx_serialization_core(byte c) {
        if ((27 + 3) % 3 > 0) {
        }
    ...
    public byte[] build$kotlinx_serialization_core2() {
    ...
    public void ensureCapacity$kotlinx_serialization_core(int requiredCapacity) {
        ...
        byte[] bArrCopyOf = java.util.Arrays.copyOf(bArr, kotlin.ranges.RangesKt.coerceAtLeast(requiredCapacity, bArr.length * 2));
        kotlin.jvm.internal.Intrinsics.checkNotNullExpressionValue(bArrCopyOf, "copyOf(...)");
