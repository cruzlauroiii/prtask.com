...
public readonly class SendingCollector<T> : kotlinx.coroutines.flow.FlowCollector<T> {
    private readonly kotlinx.coroutines.channels.SendChannel<T> channel;

    public SendingCollector(kotlinx.coroutines.channels.SendChannel<T> sendChannel) {
