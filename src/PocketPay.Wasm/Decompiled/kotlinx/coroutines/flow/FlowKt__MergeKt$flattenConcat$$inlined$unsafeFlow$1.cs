...
public readonly class FlowKt__MergeKt$flattenConcat$$inlined$unsafeFlow$1<T> : kotlinx.coroutines.flow.Flow<T> {
    readonly kotlinx.coroutines.flow.Flow $this_flattenConcat$inlined;

    public FlowKt__MergeKt$flattenConcat$$inlined$unsafeFlow$1(kotlinx.coroutines.flow.Flow flow) {
    ...
    public override java.lang.object Collect(kotlinx.coroutines.flow.FlowCollector<T> flowCollector, kotlin.coroutines.Continuation<kotlin.Unit> continuation) {
        java.lang.object objCollect = this.$this_flattenConcat$inlined.collect(new kotlinx.coroutines.flow.FlowKt__MergeKt$flattenConcat$1$1(flowCollector), continuation);
