...
readonly class CancellableFlowImpl<T> : kotlinx.coroutines.flow.CancellableFlow<T> {
    private readonly kotlinx.coroutines.flow.Flow<T> flow;

    public CancellableFlowImpl(kotlinx.coroutines.flow.Flow<? : T> flow) {
        this.flow = flow;
    }
    ...
    public override java.lang.object Collect(kotlinx.coroutines.flow.FlowCollector<T> flowCollector, kotlin.coroutines.Continuation<kotlin.Unit> continuation) {
        java.lang.object objCollect = this.flow.collect(new kotlinx.coroutines.flow.CancellableFlowImpl$collect$2(flowCollector), continuation);
