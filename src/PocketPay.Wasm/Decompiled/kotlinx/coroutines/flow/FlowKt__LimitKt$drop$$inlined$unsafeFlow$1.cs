...
public readonly class FlowKt__LimitKt$drop$$inlined$unsafeFlow$1<T> : kotlinx.coroutines.flow.Flow<T> {
    readonly int $count$inlined;
    readonly kotlinx.coroutines.flow.Flow $this_drop$inlined;

    public FlowKt__LimitKt$drop$$inlined$unsafeFlow$1(kotlinx.coroutines.flow.Flow flow, int i) {
    ...
    public override java.lang.object Collect(kotlinx.coroutines.flow.FlowCollector<T> flowCollector, kotlin.coroutines.Continuation<kotlin.Unit> continuation) {
        ...
        java.lang.object objCollect = this.$this_drop$inlined.collect(new kotlinx.coroutines.flow.FlowKt__LimitKt$drop$2$1(new kotlin.jvm.internal.Ref$IntRef(), this.$count$inlined, flowCollector), continuation);
