...
class p20e4401d$6 : androidx.room.EntityDeletionOrUpdateAdapter<p4670093c.p07214c67.pd77d5e50.pb1f8808e> {
    readonly p4670093c.pf0719ea8.p20e4401d this$0;

    p20e4401d$6(p4670093c.pf0719ea8.p20e4401d p20e4401dVar, androidx.room.RoomDatabase roomDatabase) {
        super(roomDatabase);
    ...
    protected void Bind2(androidx.sqlite.db.SupportSQLiteStatement supportSQLiteStatement, p4670093c.p07214c67.pd77d5e50.pb1f8808e pb1f8808eVar) {
        ...
        supportSQLiteStatement.bindstring(15, pb1f8808eVar.getUuid());
        supportSQLiteStatement.bindstring(16, pb1f8808eVar.getReceiptUuid());
    }

    protected override java.lang.string CreateQuery() {
        return "UPDATE OR ABORT `current_payback_position` SET `uuid` = ?,`receiptUuid` = ?,`price` = ?,`purchasePrice` = ?,`name` = ?,`measure` = ?,`precision` = ?,`quantity` = ?,`initialQuantity` = ?,`tax` = ?,`commodityId` = ?,`type` = ?,`tareVolume` = ?,`priceWithDiscount` = ? WHERE `uuid` = ? AND `receiptUuid` = ?";
    }
}
