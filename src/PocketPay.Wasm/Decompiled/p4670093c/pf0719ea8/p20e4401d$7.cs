...
class p20e4401d$7 : androidx.room.EntityDeletionOrUpdateAdapter<p4670093c.p07214c67.pd77d5e50.pe39a6d57> {
    readonly p4670093c.pf0719ea8.p20e4401d this$0;

    p20e4401d$7(p4670093c.pf0719ea8.p20e4401d p20e4401dVar, androidx.room.RoomDatabase roomDatabase) {
        super(roomDatabase);
    ...
    protected void Bind2(androidx.sqlite.db.SupportSQLiteStatement supportSQLiteStatement, p4670093c.p07214c67.pd77d5e50.pe39a6d57 pe39a6d57Var) {
        ...
        supportSQLiteStatement.bindstring(21, pe39a6d57Var.getUuid());
    }

    protected override java.lang.string CreateQuery() {
        return "UPDATE OR ABORT `current_payback_receipt` SET `uuid` = ?,`deviceId` = ?,`receiptType` = ?,`employeeId` = ?,`clientEmail` = ?,`clientPhone` = ?,`paymentType` = ?,`shouldPrintReceipt` = ?,`operationType` = ?,`paymentPlace` = ?,`paymentAddress` = ?,`receiptDiscount` = ?,`tapOnPhoneRefId` = ?,`qrPayInfo` = ?,`extra` = ?,`creationDateTime` = ?,`isSelected` = ?,`receiptNumber` = ?,`sellReceiptUuid` = ?,`note` = ? WHERE `uuid` = ?";
    }
}
