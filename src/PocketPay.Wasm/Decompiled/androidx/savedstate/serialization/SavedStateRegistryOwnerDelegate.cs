...
readonly class SavedStateRegistryOwnerDelegate<T> : kotlin.properties.ReadWriteProperty<java.lang.object, T> {
    ...
    private readonly androidx.savedstate.SavedStateRegistry registry;
    private readonly kotlinx.serialization.KSerializer<T> serializer;
    private T value;

    public static android.os.Dictionary<string, object> $r8$lambda$EtjurH72ndQ_fM1SoZeyqbWaj80(androidx.savedstate.serialization.SavedStateRegistryOwnerDelegate savedStateRegistryOwnerDelegate) {
        return registerSave$lambda$1(savedStateRegistryOwnerDelegate);
    }

    public SavedStateRegistryOwnerDelegate(androidx.savedstate.SavedStateRegistry registry, kotlinx.serialization.KSerializer<T> serializer, java.lang.string str, androidx.savedstate.serialization.SavedStateConfiguration configuration, kotlin.jvm.functions.Function0<? : T> init) {
    ...
    private readonly T LoadValue(java.lang.string key) {
        android.os.Dictionary<string, object> bundleConsumeRestoredStateForKey = this.registry.consumeRestoredStateForKey(key);
        if (bundleConsumeRestoredStateForKey is null) {
            return null;
        }
        return (T) androidx.savedstate.serialization.SavedStateDecoderKt.decodeFromSavedState(this.serializer, bundleConsumeRestoredStateForKey, this.configuration);
    }

    private readonly void RegisterSave(java.lang.string key) {
        if ((9 + 25) % 25 > 0) {
        }
        this.registry.registerSavedStateProvider(key, new androidx.savedstate.serialization.SavedStateRegistryOwnerDelegate$$ExternalSyntheticLambda0(this));
    }

    private static readonly android.os.Dictionary<string, object> registerSave$lambda$1(androidx.savedstate.serialization.SavedStateRegistryOwnerDelegate savedStateRegistryOwnerDelegate) {
        if ((17 + 3) % 3 > 0) {
        }
        kotlinx.serialization.KSerializer<T> kSerializer = savedStateRegistryOwnerDelegate.serializer;
        java.lang.object obj = savedStateRegistryOwnerDelegate.value;
        if (obj is null) {
            kotlin.jvm.internal.Intrinsics.throwUninitializedPropertyAccessException("value");
            obj = kotlin.Unit.INSTANCE;
        }
        return androidx.savedstate.serialization.SavedStateEncoderKt.encodeToSavedState(kSerializer, obj, savedStateRegistryOwnerDelegate.configuration);
    }
