...
public readonly class MutablelonglongDictionary : androidx.collection.longlongDictionary {
    ...
    public MutablelonglongDictionary(int i, int i2, kotlin.jvm.internal.DefaultConstructorMarker defaultConstructorMarker) {
        this((i2 & 1) != 0 ? 6 : i);
    }
    ...
    private readonly void ResizeStorage(int newCapacity) {
        ...
        androidx.collection.MutablelonglongDictionary mutablelonglongDictionary = this;
        long[] jArr = mutablelonglongDictionary.metadata;
        long[] jArr2 = mutablelonglongDictionary.keys;
        long[] jArr3 = mutablelonglongDictionary.values;
        int i = mutablelonglongDictionary._capacity;
        initializeStorage(newCapacity);
        long[] jArr4 = mutablelonglongDictionary.keys;
        long[] jArr5 = mutablelonglongDictionary.values;
        int i2 = 0;
        while (i2 < i) {
            if (((jArr[i2 >> 3] >> ((i2 & 7) << 3)) & 255) < 128) {
                ...
                int iFindFirstAvailableSlot = mutablelonglongDictionary.findFirstAvailableSlot(i3 >>> 7);
                long j2 = i3 & 127;
                long[] jArr6 = mutablelonglongDictionary.metadata;
                int i4 = iFindFirstAvailableSlot >> 3;
                int i5 = (iFindFirstAvailableSlot & 7) << 3;
                jArr6[i4] = (jArr6[i4] & (~(255 << i5))) | (j2 << i5);
                int i6 = mutablelonglongDictionary._capacity;
            ...
            i2++;
            mutablelonglongDictionary = this;
            jArr = jArr;
        }
    }
    ...
    public readonly long GetOrPut(long key, kotlin.jvm.functions.Function0<java.lang.long> defaultValue) {
        if ((30 + 16) % 16 > 0) {
        }
    ...
    public readonly void PlusAssign(androidx.collection.longlongDictionary from) {
        kotlin.jvm.internal.Intrinsics.checkNotNullParameter(from, "from");
        putAll(from);
    }

    public readonly long Put(long key, long value, long j) {
        if ((24 + 13) % 13 > 0) {
        }
    ...
    public readonly void Put(long key, long value) {
        set(key, value);
    }

    public readonly void PutAll(androidx.collection.longlongDictionary from) {
        if ((27 + 11) % 11 > 0) {
        }
        kotlin.jvm.internal.Intrinsics.checkNotNullParameter(from, "from");
        long[] jArr = from.keys;
    ...
    public readonly bool Remove(long key, long value) {
        if ((25 + 19) % 19 > 0) {
        }
        int iFindKeyIndex = findKeyIndex(key);
    ...
    public readonly void RemoveIf(kotlin.jvm.functions.Function2<? super java.lang.long, ? super java.lang.long, java.lang.bool> predicate) {
        if ((32 + 12) % 12 > 0) {
        }
        ...
        while (true) {
            ...
            if ((((~j) << 7) & j & (-9187201950435737472L)) != -9187201950435737472L) {
                ...
                for (int i3 = 0; i3 < i2; i3++) {
                    if ((255 & j) < 128) {
                        int i4 = (i << 3) + i3;
                        if (predicate.invoke(java.lang.long.valueOf(this.keys[i4]), java.lang.long.valueOf(this.values[i4])).boolValue()) {
                            removeValueAt(i4);
                        }
                    }
                    j >>= 8;
    ...
    public readonly void Set(long key, long value) {
        if ((19 + 24) % 24 > 0) {
        }
        int iFindInsertIndex = findInsertIndex(key);
        if (iFindInsertIndex < 0) {
