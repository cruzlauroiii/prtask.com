...
Console.WriteLine("  Auth status: " + (int)R1.StatusCode);
Console.WriteLine("  Token (first 50): " + Token.Substring(0, Math.Min(50, Token.Length)));

Console.WriteLine("\nStep 2: DNS check api.psr-it.com");
try
{
    var Addresses = System.Net.Dns.GetHostAddresses("api.psr-it.com");
    foreach (var Addr in Addresses)
        Console.WriteLine("  api.psr-it.com -> " + Addr);
}
catch (Exception Ex)
{
...
Console.WriteLine("\nStep 3: status/toph on api.psr-it.com");
...
try
{
    var R2 = Client.PostAsync("https://api.psr-it.com/api/v2/device/status/toph", new StringContent(StatusBody, Encoding.UTF8, "application/json")).Result;
    var Body2 = R2.Content.ReadAsStringAsync().Result;
    if (Body2.Length > 300) Body2 = Body2.Substring(0, 300);
...
Console.WriteLine("\nStep 4: order/toph on api.psr-it.com");
var OrderBody = "{\"user_id\":\"34c9c05b-4de8-4edf-9bfa-0efdb09d8a94\",\"phone\":\"639568564236\",\"fio\":\"\",\"inn\":\"\",\"region_id\":0}";
try
{
    var R3 = Client.PostAsync("https://api.psr-it.com/api/v2/market/order/toph", new StringContent(OrderBody, Encoding.UTF8, "application/json")).Result;
...
Console.WriteLine("\nStep 5: order/toph on api.psr-it.com with v3");
try
{
    var R4 = Client.PostAsync("https://api.psr-it.com/api/v3/market/order/toph", new StringContent(OrderBody, Encoding.UTF8, "application/json")).Result;
