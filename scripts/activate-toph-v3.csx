...
var Combos = new[]
{
    ("application/json", "application/json", "no-auth"),
    ("application/json", "application/jwt", "no-auth"),
    ("application/jwt", "application/jwt", "no-auth"),
    ("application/json", "application/json", "bearer"),
    ("application/json", "application/jwt", "bearer"),
    ("application/json", "*/*", "bearer"),
};

foreach (var (ContentType, Accept, Auth) in Combos)
{
    ...
    if (Auth == "bearer") Client.DefaultRequestHeaders.TryAddWithoutValidation("Authorization", "Bearer " + TophToken);

    try
    {
        var Content = new StringContent(ActivationReq, Encoding.UTF8, ContentType);
        var R = Client.PostAsync("https://mob-gcash.t2ph.psr-it.com/api/mt/activations", Content).Result;
        var Body = R.Content.ReadAsStringAsync().Result;
        Console.WriteLine($"CT={ContentType} Accept={Accept} Auth={Auth} => {(int)R.StatusCode} {(Body.Length > 100 ? Body[..100] : Body)}");
    }
    catch (Exception Ex) { Console.WriteLine($"CT={ContentType} Accept={Accept} Auth={Auth} => ERROR {Ex.InnerException?.Message ?? Ex.Message}"); }
}
