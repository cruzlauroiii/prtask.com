...
if (!RespStr.Contains("101")) { Console.WriteLine("Handshake failed: " + RespStr); return; }
...
WsSend("{\"Method\":\"ReadCard\",\"Id\":\"1\"}");
Console.WriteLine("Sent ReadCard command");

var Resp = WsReceive();
Console.WriteLine("Response: " + Resp);

Console.WriteLine("Waiting for CardData from Companion (tap card on phone)...");
for (var I = 0; I < 6; I++)
{
    try
    {
        ...
        var Msg = WsReceive();
        Console.WriteLine("Received: " + (Msg.Length > 200 ? Msg[..200] + "..." : Msg));
        if (Msg.Contains("CardData")) break;
    }
    catch (Exception Ex) { Console.WriteLine("Timeout: " + Ex.Message); }
}
